...
function search() {
  ...
  var searchProperties = [r.slf, r.name];
  var resultsArray = data.filter(function (r) {
    return searchWords.every(function (word) {
      return [r.slf, r.name].some(function (property) {
        return r.slf.toString().toLowerCase().indexOf(word) !== -1;
      });
    }); // return r.slf.toString().toLowerCase().indexOf(searchWords) !== -1;
  });
