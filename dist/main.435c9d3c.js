...
function editHandcraft() {
  ...
  editedHandcraft.retail = $("#edit-retail")[0].value; // let editChipEl = document.getElementById("edit-tags");
  // let chip = M.Chips.init(editChipEl);
  // let chipsData = chip.chipsData.map(c => { return c.tag });
  // editedHandcraft.tags = chipsData.join(", ");
  // console.log(chip.chipsData);
  // let tags = $('#edit-tags').chips('data');
  // console.log(tags);
...
function clickEventHandler(e) {
  if (e.target.matches(".edit-button")) {
    ...
    google.script.run.withSuccessHandler(function (editHandcraft) {
      ...
      $("#edit-description")[0].value = editHandcraft.description; // $("#edit-tags")[0].value = editHandcraft.tags;

      $("#edit-notes")[0].value = editHandcraft.notes; //handle tags

      var tagsArray = editHandcraft.tags.split(',');
      var tagData = [];
