...
function search() {
  ...
  var resultsArray = data.filter(function (r) {
    return searchWords.every(function (word) {
      [r.slf, r.name].some(function (searchProperty) {
        return searchProperty.toString().toLowerCase().indexOf(word) !== -1;
      });
    }); // return r.name.toString().toLowerCase().indexOf(searchInput) !== -1;
  });
